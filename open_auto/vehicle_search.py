...
class Ui_Form(object):
   #### collect vin data
    ...
    def save_vehicle(self):
        self.customer_info = QtWidgets.QWidget()
        self.customer_info_ui = new_customer.Ui_create_customer_form()
        self.customer_info_ui.setupUi(self.customer_info)
        if self.customer_info_ui.save_button.clicked:
        ...
        add_options = """UPDATE customers SET vin = %s, year = %s, 
        make = %s, model = %s, engine = %s, trim = %s ORDER BY id DESC LIMIT 1"""
        conn.execute(add_options, vehicle_attributes)
        my_db.commit()
        my_db.close()
    ...
    def edit_vehicle(self):
        ...
        name = [self.customer_info_ui.name_line.text(), ]
        vehicle_attributes = [self.vin_line.text(), self.year_line.text(), self.make_line.text(), self.model_line.text(),
                              self.engine_line.text(), self.trim_line.text(), self.customer_info_ui.name_line.text()]
        print(vehicle_attributes)
        conn = my_db.cursor()
        change_vehicle = """UPDATE CUSTOMERS.customers SET vin = %s, year = %s, make = %s, model = %s,
                                engine = %s, trim = %s WHERE name = %s"""
        conn.execute(change_vehicle, (vehicle_attributes[0], vehicle_attributes[1], vehicle_attributes[2],
                     vehicle_attributes[3], vehicle_attributes[4], vehicle_attributes[5], name[0]))
        my_db.commit()
        my_db.close()
...
#        Form.setStyleSheet("  background-color: #504F4F;")
        ...
        icon3 = QtGui.QIcon()
        icon3.addPixmap(QtGui.QPixmap("../ui_files/Images/Icons/check.png"), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.On)
        ...
        icon4 = QtGui.QIcon()
        icon4.addPixmap(QtGui.QPixmap("../ui_files/Images/Icons/check.png"), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.On)
